...
dt <- read_parquet('data/dt_clean.parquet')
user_outcomes <- read_parquet('data/user_outcomes.parquet')
```
...
# sort data by time
setorder(monthly_train_data, ds)
...
m <- fit.prophet(m, monthly_train_data)

# Calculate MONTHS needed to reach Dec 2024
last_ds <- max(monthly_train_data$ds)
...
prophet_plot_components(m, forecast_daily)
...
# Plot 2: The Components Breakdown
prophet_plot_components(m, forecast_monthly)
