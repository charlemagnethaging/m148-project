...
author: "Isaiah C. Mireles, Joel Yoon, Charlie Kopp"
date: "2026-05-06"
date-format: long
format:
  pdf:
...
# Task 3 - Time series # orders shipped


- Important observations from aggregation process:
  - the data doesn't include any ongoing journeys at its start point $\Rightarrow$ there aren't any orders shipped until a few days in. **Solution:** find 80th percentile of successful journey length and start train data there to accurately capture a reflective trend
  - Even though the warehouse is closed on weekends, the data shows orders shipped on quite a few weekends (at a much lower count than weekdays). For daily aggregation, this would show a misleading weekly seasonality effect. **Solution:** (*probably not the best solution!*) drop all weekends from the dataset, the Prophet model interpolates missing dates.
- Without extra information, I would probably use weekly aggregation because of the issue with orders shipped on weekends. It is still reflective of granular seasonality, specifically holidays, which have the biggest impact on orders shipped.
- The most useful additional regressor to use would be seasonal promotions (we don't have that AFAIK).

## Plots:

![Prophet Daily Forecast plot](charlie/prophet-plots/daily_forecast.png)

![Prophet Weekly Forecast plot](charlie/prophet-plots/weekly_forecast.png)

![Prophet Monthly Forecast plot](charlie/prophet-plots/monthly_forecast.png)
